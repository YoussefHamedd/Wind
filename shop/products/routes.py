from flask import redirect, url_for, request, flash, render_template, session, current_app, jsonify, Flask
...
def home():
    ...
    return render_template('home.html', title="Store Home", products=products, brands=get_all_brands(), categories=get_all_categories())

@app.route('/about')
def about():
    page = request.args.get('page', 1, type=int)
    return render_template('about.html', title="about", products=products, brands=get_all_brands(), categories=get_all_categories())

@app.route('/contact')
def contact():
    page = request.args.get('page', 1, type=int)
    return render_template('contact.html', title="contact", products=products, brands=get_all_brands(), categories=get_all_categories())

@app.route('/products')
def products():
    page = request.args.get('page', 1, type=int)
    products = Product.query.filter(Product.stock > 0).paginate(page=page, per_page=10)
    return render_template('products/products.html', title="Store Home", products=products, brands=get_all_brands(), categories=get_all_categories())
...
def get_brand(id):
    ...
    return render_template('products/products.html', brand=brand, title=Brand.query.get(id).name, brands=get_all_brands(),
...
def get_category(id):
    ...
    return render_template('products/products.html', category=category, title=Category.query.get(id).name,
...
def service():
    page = request.args.get('page', 1, type=int)
    products = Product.query.filter(Product.stock > 0).paginate(page=page, per_page=4)
    return render_template('services.html', title="Forcast", products=products, brands=get_all_brands(), categories=get_all_categories())
...
def predict():
    ...
    di_dict = {'E': 1, 'W': 2, 'N': 3, 'S': 4, 'NE': 5, 'NW': 6, 'SE': 7, 'SW': 8}
    converted_wind_direction = di_dict.get(di, 0)
    wind_speeds = [10, 20, 30, 40, 50, 100, 120, 142, 170]
    predictions = []

    for speed in wind_speeds:
        prediction = model.predict([[speed, converted_wind_direction]])
        predictions.append(prediction[0][0])

    print(wind_speeds)
    print(predictions)

    return render_template('services.html', prediction_text='The power to be generated is {} Mega Watts'.format(output),
                           a=result, wind_speeds=wind_speeds, predictions=predictions)
